...
function u1.updateTeam(p10, p11, p12)
	...
	if p11 == l__Players__6.LocalPlayer then
		if l__PlaceUtil__4.isLobbyServer() then
			...
			local l__Nametag__45 = p12:WaitForChild("Head"):WaitForChild("Nametag");
			print("destroying nametag for player:", p11);
			l__Nametag__45.PlayerToHideFrom = l__Players__6.LocalPlayer;
		end;
	end;
	if l__PlaceUtil__4.isGameServer() and v43 then
		local l__TeamIndicator__46 = p12:WaitForChild("Head"):WaitForChild("Nametag"):WaitForChild("TeamIndicator");
		l__TeamIndicator__46.Visible = true;
		l__TeamIndicator__46.BackgroundColor3 = v43.color;
	end;
	if l__PlaceUtil__4.isGameServer() then
		local v47 = l__EntityUtil__7:getEntity(p11);
		if v47 then
			if v38 == v39 then
				local v48 = l__Theme__8.mcGreen;
			else
				v48 = Color3.fromRGB(240, 26, 26);
			end;
			v47:setNametag("<font color=\"#" .. v48:ToHex() .. "\">" .. p11.DisplayName .. "</font>");
			v47:setNametagBorderColor(v48);
		end;
	end;
end;
