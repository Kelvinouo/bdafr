...
local u7 = v1.import(script, script.Parent, "glitched-match-announcement").GlitchedMatchAnnouncement;
...
local l__LuckyBlockCategory__12 = v1.import(script, game:GetService("ReplicatedStorage"), "TS", "lucky-block", "lucky-block-enums").LuckyBlockCategory;
...
function v5.KnitStart(p2)
	...
	if l__PlaceUtil__4.isGameServer() then
		l__default__5.Client:OnEvent("RemoteName", function()
			local u14 = u6.mount(u6.createElement("ScreenGui", {}, { u6.createElement(u7) }), l__Players__8.LocalPlayer:FindFirstChildOfClass("PlayerGui"));
			task.delay(l__BalanceFile__9.GLITCH_LB_ANNOUNCEMENT_DURATION, function()
				l__SoundManager__10:playSound(l__GameSound__3.GLITCH_OVERLAY_2, {
	...
	l__WatchCollectionTag__11("GlitchedLuckyBlock", function(p3)
		p2:playGlitchOverlayEffect(p3);
		p2:loopGlitchParticlesOnBlock(p3);
	end);
	l__default__5.Client:OnEvent("RemoteName", function(p4)
		if p4.category ~= l__LuckyBlockCategory__12.GLITCHED then
			return nil;
		end;
		l__SoundManager__10:playSound(l__GameSound__3.GLITCHED_LUCKY_BLOCK_DAMAGE);
		p2:playGlitchParticlesOnBlock(p4.blockPosition);
	end);
	l__ClientSyncEvents__13.PlaceBlock:connect(function(p5)
		if p5.blockType ~= l__ItemType__2.GLITCHED_LUCKY_BLOCK then
			return nil;
		end;
		p2:playGlitchParticlesOnBlock(p5.blockPosition);
		l__SoundManager__10:playSound(l__GameSound__3.TV_STATIC, {
			playbackSpeedMultiplier = math.random(0.5, 1.5), 
			volumeMultiplier = 0.05
		});
	end);
	l__ClientSyncEvents__13.DamageBlock:connect(function(p6)
	...
	l__default__5.Client:OnEvent("RemoteName", function(p7)
		l__SoundManager__10:playSound(l__GameSound__3.GLITCHED_LUCKY_BLOCK_DAMAGE);
		p2:playGlitchedLuckyBlockCloneEffect(p7.initialPosition, true);
		wait(l__BalanceFile__9.GLITCH_LB_TELEPORT_DURATION);
		l__SoundManager__10:playSound(l__GameSound__3.GLITCHED_LUCKY_BLOCK_TELEPORT);
		p2:playGlitchedLuckyBlockCloneEffect(p7.newPosition, false);
	end);
end;
local u15 = v1.import(script, v1.getModule(script, "@rbxts", "make"));
local l__SpriteSheetPlayer__16 = v2.SpriteSheetPlayer;
...
function v5.playGlitchOverlayEffect(p8, p9)
	...
	local function v8(p10)
		local u20 = l__SpriteSheetPlayer__16.new(u15("ImageLabel", {
			Size = UDim2.fromScale(1, 1), 
			BackgroundTransparency = 1, 
			Parent = u15("SurfaceGui", {
				Name = "SpriteSheet_Player_" .. p10.Name, 
				Face = p10, 
				Brightness = 3, 
				Parent = p9
			})
		}), l__RandomUtil__17.randomArraySelectN(u18, 1)[1]);
		task.delay(u19:NextNumber(0, 2), function()
			u20:play();
		end);
	end;
	...
	local function v13(p11)
		...
		while true do
			...
			local v19 = u15("ImageLabel", {
				Size = UDim2.new(0, u19:NextInteger(1, 3), 0, 1), 
				Position = UDim2.fromOffset(u19:NextInteger(0, 8), u19:NextInteger(0, 8)), 
				BackgroundColor3 = v15[u19:NextInteger(0, #v15 - 1) + 1], 
				BorderSizePixel = 0, 
				Parent = v14
			});		
		end;
...
local l__ReplicatedStorage__22 = v3.ReplicatedStorage;
local l__Workspace__23 = v3.Workspace;
local l__BlockEngine__24 = v1.import(script, v1.getModule(script, "@easy-games", "block-engine").out).BlockEngine;
local l__EffectUtil__25 = v1.import(script, script.Parent.Parent.Parent.Parent, "lib", "effect", "effect-util").EffectUtil;
function v5.playGlitchParticlesOnBlock(p12, p13, p14, p15)
	task.spawn(function()
		local v23 = l__ReplicatedStorage__22.Assets.Effects.GlitchedLuckyBlockEffect:Clone();
		v23.Name = "GlitchedLuckyBlockEffect";
		v23.Parent = l__Workspace__23;
		v23.Transparency = 1;
		v23.CFrame = CFrame.new((l__BlockEngine__24:getWorldPosition(p13)));
		local v24 = {
			destroyAfterSec = 1.5
		};
		local v25 = p14;
		...
		l__EffectUtil__25:playEffects({ v23 }, nil, v24);
	end);
end;
function v5.playGlitchedLuckyBlockCloneEffect(p16, p17, p18)
	task.spawn(function()
		if p18 then
			local v27 = l__ReplicatedStorage__22.Assets.Effects.GlitchedLuckyBlockTeleportStart:Clone();
		else
			v27 = l__ReplicatedStorage__22.Assets.Effects.GlitchedLuckyBlockTeleportEnd:Clone();
		end;
		v27.Name = "GlitchedLuckyBlockEffect";
		v27.Parent = l__Workspace__23;
		v27.Transparency = 1;
		v27.CFrame = CFrame.new((l__BlockEngine__24:getWorldPosition(p17)));
		local v28 = {};
		if p18 then
			local v29 = 2;
		else
		...
		l__EffectUtil__25:playEffects({ v27 }, nil, v28);
	end);
end;
