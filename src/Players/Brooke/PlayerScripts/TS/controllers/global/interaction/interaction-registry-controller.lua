...
function u1.RegisterInteraction(p3, p4)
	...
	l__CollectionService__3:GetInstanceRemovedSignal(p4.interactionTag):Connect(function(p6)
		...
		if v10 then
			v10:Destroy();
			print("Removed interaction for part \"" .. p6:GetFullName() .. "\" with interaction tag \"" .. p4.interactionTag .. "\"");
		end;
	end);
	if p4.onVisibilityStateChanged then
		p4.onVisibilityStateChanged:Connect(function(p7)
			...
			if v11 then
				...
				print("Updated visibility of interaction \"" .. p4.interactionTag .. "\" to state \"" .. tostring(p7) .. "\"");
			end;
		end);
	end;
