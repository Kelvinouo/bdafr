...
function v5.KnitStart(p2)
	...
	l__ClientSyncEvents__1.ProjectileLaunched:connect(function(p3)
		...
		p3.projectile:FindFirstChildOfClass("AnimationController"):LoadAnimation(l__GameAnimationUtil__2.getAnimation(l__AnimationType__3.GHOST_SPIN)):Play(0.05);
	end);
end;
...
function v5.validCollectableEntityTypes(p4)
	return { l__CollectableEntityType__4.GHOST };
end;
local l__ReplicatedStorage__5 = v3.ReplicatedStorage;
function v5.pickDisplayModel(p5, p6, p7)
	return {
		repStorageModel = l__ReplicatedStorage__5:WaitForChild("Assets"):WaitForChild("Misc"):WaitForChild("Ghost"), 
		collectionTag = p6
	};
end;
...
function v5.onWillRemoveEntity(p8, p9, p10, p11, p12)
	...
	if p11 then
		local v8 = p12:FindFirstChildOfClass("AnimationController"):LoadAnimation(l__GameAnimationUtil__2.getAnimation(l__AnimationType__3.GHOST_SPIN));
		v8:Play(0.3, nil, 0.2);
		l__default__6(1.4, l__InQuad__7, function(p13)
			local v9 = p11;
			if v9 ~= nil then
				v9 = v9.Character;
				if v9 ~= nil then
			...
			local v10 = v9;
			if p11 and v10 then
				local l__hand__11 = l__InventoryUtil__8.getInventory(p11).hand;
				local v12 = l__hand__11;
				if v12 ~= nil then
					v12 = v12.tool;
				end;
				local v13 = v12 and l__hand__11.itemType == l__ItemType__9.VACUUM;
				if v13 then
	...
	if p11 == l__Players__10.LocalPlayer then
		l__KnitClient__11.Controllers.VacuumController:setLoaded();
		l__GameAnimationUtil__2.playAnimation(l__Players__10.LocalPlayer, l__AnimationType__3.VACUUM_GHOST_CAPTURED, {
			looped = false
		});
	end;
	return nil;
...
function v5.onCreatedEntity(p14, p15, p16, p17)
	...
	if v14 ~= nil then
		v14:LoadAnimation(l__GameAnimationUtil__2.getAnimation(l__AnimationType__3.GHOST_IDLE)):Play();
		task.spawn(function()
			while v1.Promise.delay(math.random(15, 25)):await() and p17.Parent do
				v14:LoadAnimation(l__GameAnimationUtil__2.getAnimation(l__AnimationType__3.GHOST_FLIP)):Play();			
			end;
		end);
	end;
	return nil;
