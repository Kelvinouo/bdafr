...
function v4.validCollectableEntityTypes(p3)
	return { l__CollectableEntityType__2.STARS };
end;
local l__ReplicatedStorage__3 = v2.ReplicatedStorage;
function v4.pickDisplayModel(p4, p5, p6)
	local v6 = p4:getRandomStar();
	return {
		repStorageModel = l__ReplicatedStorage__3:WaitForChild("Assets"):WaitForChild("Misc"):WaitForChild(v6), 
		collectionTag = p5, 
		proximityPrompt = {
			name = v6
		}
...
function v4.onCreatedEntity(p12, p13, p14, p15)
	...
	l__SoundManager__4:playSound(l__GameSound__5.STAR_IDLE, {
		looped = true, 
	...
	if v7 then
		l__GameAnimationUtil__6.playAnimation(v7, l__AnimationType__7.STAR_IDLE);
		task.spawn(function()
			while v1.Promise.delay(math.random(15, 25)):await() and p15.Parent do
				l__GameAnimationUtil__6.playAnimation(v7, l__AnimationType__7.STAR_RANDOM_SPIN);			
			end;
		end);
	end;
end;
...
function v4.collectEntity(p16, p17, p18, p19)
	if p17 == l__Players__8.LocalPlayer then
		l__SoundManager__4:playSound(l__GameSound__5.STAR_PICKUP);
