...
function v4.isRelevantItem(p2, p3)
	return p3.itemType == l__ItemType__2.TRUMPET;
end;
...
function v4.onEnable(p4, p5, p6)
	local u6 = l__Flamework__3.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):enableAbility(l__AbilityId__4.TRUMPET_PLAY, nil);
	p4.abilityMaid:GiveTask(function()
		u6.Destroy();
		l__ContextActionService__5:UnbindAction("use-trumpet");
	end);
	l__ContextActionService__5:BindAction("use-trumpet", function(p7, p8, p9)
		if p8 == Enum.UserInputState.Begin then
			if l__Flamework__3.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):canUseAbility(l__AbilityId__4.TRUMPET_PLAY) then
				l__Flamework__3.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):useAbility(l__AbilityId__4.TRUMPET_PLAY);
				return;
			end;
		else
			return;
...
local l__EffectUtil__15 = v1.import(script, script.Parent.Parent.Parent.Parent.Parent, "lib", "effect", "effect-util").EffectUtil;
...
function v4.KnitStart(p11)
	...
	l__KnitClient__7.Controllers.PreloadController:preloadForItemType(l__ItemType__2.TRUMPET, {
		sounds = { l__GameSound__8.TRUMPET_PLAY }
	});
	l__ClientSyncEvents__9.AbilityUsed:connect(function(p12)
		local v6 = false;
		if p12.ability == l__AbilityId__4.TRUMPET_PLAY then
