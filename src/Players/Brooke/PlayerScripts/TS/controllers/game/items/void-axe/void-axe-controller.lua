...
local l__DamageType__6 = v1.import(script, game:GetService("ReplicatedStorage"), "TS", "damage", "damage-type").DamageType;
function v6.KnitStart(p2)
	l__HandKnitController__5.KnitStart(p2);
	l__default__2.Client:OnEvent("RemoteName", function(p3)
		if p3.entity.PrimaryPart then
			if p3.entity ~= l__Players__3.LocalPlayer.Character then
				l__SoundManager__4:playSound(l__GameSound__5.VOID_AXE_LEAP, {
	...
	l__default__2.Client:OnEvent("RemoteName", function(p4)
		if p4.damageType == l__DamageType__6.VOID_AXE_SLASH then
			local v8 = {};
			if l__Players__3.LocalPlayer.Character == p4.entityInstance then
				local v9 = nil;
			else
			...
			v8.position = v9;
			l__SoundManager__4:playSound(l__GameSound__5.VOID_AXE_HIT, v8);
		end;
	end);
end;
...
function v6.useVoidAxe(p5)
	if l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):isOnCooldown(l__CooldownId__8.VOID_AXE) then
		if l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):getRemainingCooldown(l__CooldownId__8.VOID_AXE) < 0.25 then
			l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):registerBufferedCallback(l__CooldownId__8.VOID_AXE, "buffer", function()
	...
	if not l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):canUseAbility(l__AbilityId__9.VOID_AXE_JUMP) then
		return nil;
	end;
	l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):useAbility(l__AbilityId__9.VOID_AXE_JUMP);
	l__SoundManager__4:playSound(l__GameSound__5.VOID_AXE_LEAP);
	...
	local u19 = l__GameAnimationUtil__14.playAnimation(l__Players__3.LocalPlayer, l__AnimationType__13.JADE_HAMMER_SLAM);
	p5.maid:GiveTask(function()
		if u19 ~= nil then
			u19:Stop();
		end;
...
function v6.onEnable(p6, p7, p8)
	p6.maid:GiveTask(l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/ability/ability-controller@AbilityController"):enableAbility(l__AbilityId__9.VOID_AXE_JUMP, nil));
	task.spawn(function()
		local v16 = nil;
		v16 = l__GameAnimationUtil__14.playAnimation(l__Players__3.LocalPlayer, l__AnimationType__13.JADE_HAMMER_IDLE, {
			looped = true
		});
		if p8() then
...
local v17 = l__KnitClient__12.CreateController(v6.new());
return nil;
