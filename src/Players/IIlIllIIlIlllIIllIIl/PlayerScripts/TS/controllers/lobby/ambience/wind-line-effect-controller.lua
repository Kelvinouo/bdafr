...
v3.Name = "WindTrailAmbientEffects";
v3.Parent = v2.Workspace;
...
local l__getConfig__3 = v1.import(script, game:GetService("ReplicatedStorage"), "TS", "_config", "config").getConfig;
function u1.KnitStart(p2)
	if l__getConfig__3("Lobby") then
		return;
	end;
	return nil;
end;
...
function u1.followPath(p3, p4, p5)
	...
	while true do
		...
		local v9 = l__TweenService__4:Create(p4, TweenInfo.new(0.05 + 0.05 * math.pow((v7 / #p5 - 0.5) * 4, 2), Enum.EasingStyle.Linear), {
			Position = p5[v7 + 1]
		});
		v9:Play();
		v9.Completed:Wait();	
...
local u6 = Vector3.new(0.1, 0.1, 0.075);
local l__WIND_DIRECTION__7 = v1.import(script, script.Parent, "ambience-settings").WIND_DIRECTION;
local u8 = v1.import(script, v1.getModule(script, "@rbxts", "make"));
local l__NumberSequenceBuilder__9 = v1.import(script, script.Parent, "number-sequence-builder").NumberSequenceBuilder;
function u1.spawnWindParticle(p6)
	v1.Promise.defer(function()
		local v10 = l__Players__5.LocalPlayer;
		if v10 ~= nil then
			v10 = v10.Character;
			if v10 ~= nil then
				v10 = v10.PrimaryPart;
		...
		local l__Unit__12 = (l__WIND_DIRECTION__7 + u6 * (math.random() * 2 - 1)).Unit;
		...
		u8("Trail", {
			FaceCamera = true, 
			LightInfluence = 0.3, 
			Transparency = l__NumberSequenceBuilder__9.new():addKeypoint(0, 0.85):build(), 
			Lifetime = 0.65, 
			WidthScale = l__NumberSequenceBuilder__9.new():addKeypoint(0, 0.2):addKeypoint(0.5, 1):addKeypoint(1, 0.2):build(), 
