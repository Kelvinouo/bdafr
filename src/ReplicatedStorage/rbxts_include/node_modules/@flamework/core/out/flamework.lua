-- Script Hash: 14e789959571899ed9171a2e57150dcba2e74660b5c407d67bea312a0116c32b285b0878a7bfe8b1ba4e677a7c9dde59
...
function u11.ignite(p10)
	...
	for v40, v41 in pairs(l__Modding__3.getSingletons()) do
		local v42 = l__Modding__3.getDecorator(v40, nil, v39);
		if v42 and (not l__Reflect__1.getOwnMetadata(v40, "flamework:isExternal") or u6[v40] ~= nil) then
			table.insert(v38, { l__Modding__3.resolveSingleton(v40), v42.arguments[1] or {} });
		end;
	end;
	local v43 = {};
	local v44 = {};
	table.sort(v38, function(p11, p12)
		local v45 = p11[2].loadOrder;
		if v45 == nil then
			v45 = 1;
		end;
		local v46 = p12[2].loadOrder;
		if v46 == nil then
			v46 = 1;
		end;
		return v45 < v46;
	...
	while true do
		...
		local v52 = v51[1];
		if v3._implements(v52, "$:flamework@OnInit") then
			table.insert(v44, v52);
		end;
		if v3._implements(v52, "$:flamework@OnStart") then
			table.insert(v43, v52);
		end;	
	end;
	for v53, v54 in ipairs(v44) do
		debug.setmemorycategory(l__Reflect__1.getMetadata(v54, "identifier"));
		local v55 = v54:onInit();
		if v1.Promise.is(v55) then
			v55:await();
		end;
	...
	l__RunService__4.Heartbeat:Connect(function(p19)
		for v56, v57 in pairs(u12) do
			task.spawn(function()
				return v56:onTick(p19);
			end);
		end;
	end);
	...
	if l__RunService__4:IsClient() then
		l__RunService__4.RenderStepped:Connect(function(p22)
			for v60, v61 in pairs(u14) do
				task.spawn(function()
					return v60:onRender(p22);
				end);
			end;
		end);
	end;
	for v62, v63 in ipairs(v43) do
		task.spawn(function()
			debug.setmemorycategory(l__Reflect__1.getMetadata(v63, "identifier"));
			v63:onStart();
		end);
	end;
	return v38;
...
u11.Testing = {
	patchDependency = function(p23, p24)
		...
		local v64 = l__Reflect__1.idToObj[p24];
		if v64 == nil then
			error("Dependency " .. p24 .. " was not found and cannot be patched.");
		end;
		if l__Modding__3.getSingletons()[v64] ~= nil then
			error(p24 .. " has already been resolved, continuing is unsafe");
		end;
		local v65 = l__Reflect__1.metadata[v64];
		if not v65 then
			error("Dependency " .. p24 .. " has no existing metadata.");
		end;
		l__Reflect__1.defineMetadata(v64, "flamework:isPatched", true);
		l__Reflect__1.metadata[v64] = nil;
		l__Reflect__1.metadata[p23] = v65;
