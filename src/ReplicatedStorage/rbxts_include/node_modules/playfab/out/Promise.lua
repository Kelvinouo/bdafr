-- Script Hash: 4bc89e050c62e6819e6fbc8c58f0a2a7cc88cf522571e530f8fa80d7b1de0dc6baf2255603c1a34d058564b06cb81291
...
v13.prototype.cancel = function(p92)
	...
	for v92 in pairs(p92._consumers) do
		v92:cancel();
	end;
	p92:_finalize();
end;
...
v13.prototype._finally = function(p95, p96, p97, p98)
	...
	return v13._new(p96, function(p99, p100)
		...
		if p95._status == v13.Status.Started then
			table.insert(p95._queuedFinally, v93);
			return;
		end;
		v93(p95._status);
	end, p95);
...
v13.prototype.finally = function(p101, p102)
	local v94 = true;
	if p102 ~= nil then
		v94 = type(p102) == "function";
	end;
	assert(v94, string.format("Please pass a handler function to %s!", "Promise:finally"));
...
v13.prototype.finallyCall = function(p103, p104, ...)
	...
	return p103:_finally(debug.traceback(nil, 2), function()
		return p104(unpack(v96, 1, v95));
	end);
end;
v13.prototype.finallyReturn = function(p105, ...)
	local v97, v98 = u9(...);
	return p105:_finally(debug.traceback(nil, 2), function()
		return unpack(v98, 1, v97);
	end);
end;
v13.prototype.done = function(p106, p107)
	local v99 = true;
	if p107 ~= nil then
		v99 = type(p107) == "function";
	end;
	assert(v99, string.format("Please pass a handler function to %s!", "Promise:done"));
...
local function u31(p114, ...)
	if p114 ~= v13.Status.Resolved then
		if ... == nil then
			local v104 = "Expected Promise rejected with no value.";
		else
			v104 = ...;
		end;
		error(v104, 3);
...
v13.prototype._resolve = function(p117, ...)
	...
	if not v13.is((...)) then
		...
		local v105, v106 = u9(...);
		p117._valuesLength = v105;
		p117._values = v106;
		for v107, v108 in ipairs(p117._queuedResolve) do
			coroutine.wrap(v108)(...);
		end;
		p117:_finalize();
		return;
	...
	local u32 = ...;
	local v109 = u32:andThen(function(...)
		p117:_resolve(...);
	end, function(...)
		local v110 = u32._values[1];
		if u32._error then
			v110 = v6.new({
				error = u32._error, 
				kind = v6.Kind.ExecutionError, 
		...
		if not v6.isKind(v110, v6.Kind.ExecutionError) then
			p117:_reject(...);
			return;
		end;
		return p117:_reject(v110:extend({
			error = "This Promise was chained to a Promise that errored.", 
			trace = "", 
			context = string.format("The Promise at:\n\n%s\n...Rejected because it was chained to the following Promise, which encountered an error:\n", p117._source)
		}));
	end);
	if v109._status == v13.Status.Cancelled then
		p117:cancel();
		return;
	end;
	if v109._status == v13.Status.Started then
		p117._parent = v109;
		v109._consumers[p117] = true;
	end;
end;
...
v13.prototype._reject = function(p119, ...)
	...
	local v111, v112 = u9(...);
	p119._valuesLength = v111;
	p119._values = v112;
	if not u33(p119._queuedReject) then
		for v113, v114 in ipairs(p119._queuedReject) do
			coroutine.wrap(v114)(...);
		end;
	else
		local u34 = tostring((...));
...
v13.prototype._finalize = function(p120)
	for v115, v116 in ipairs(p120._queuedFinally) do
		coroutine.wrap(v116)(p120._status);
	end;
...
v13.prototype.now = function(p121, p122)
	local v117 = nil;
	v117 = debug.traceback(nil, 2);
	if p121:getStatus() == v13.Status.Resolved then
		return p121:_andThen(v117, function(...)
			return ...;
		end);
	end;
	if p122 == nil then
