...
function v80.SetSoundId(p54, p55)
	if typeof(p55) ~= "string" then
		error("bad argument #1 to 'SetSoundId' (string expected, got " .. typeof(p55) .. ")", 2);
	end;
	v25.SoundId = p55;
end;
local u21 = l__None__18;
function v80.SetCollisionMode(p56, p57, p58)
	if p57 == v13.None then
		u2 = nil;
		u3 = nil;
	elseif p57 == v13.Blacklist then
		if typeof(p58) == "Instance" then
			u2 = { p58, v26 };
		elseif typeof(p58) == "table" then
			for v124 = 1, #p58 do
				if typeof(p58[v124]) ~= "Instance" then
					error("bad argument #2 to 'SetCollisionMode' (blacklist contained a " .. typeof(p58[v124]) .. " on index " .. tostring(v124) .. " which is not an Instance)", 2);
				end;
			end;
			u2 = { v26 };
			for v125 = 1, #p58 do
				table.insert(u2, p58[v125]);
			end;
		else
			error("bad argument #2 to 'SetCollisionMode (Instance or array of Instance expected, got " .. typeof(p58) .. ")'", 2);
		end;
		u3 = nil;
	elseif p57 == v13.Whitelist then
		if typeof(p58) == "Instance" then
			u2 = { p58 };
		elseif typeof(p58) == "table" then
			for v126 = 1, #p58 do
				if typeof(p58[v126]) ~= "Instance" then
					error("bad argument #2 to 'SetCollisionMode' (whitelist contained a " .. typeof(p58[v126]) .. " on index " .. tostring(v126) .. " which is not an Instance)", 2);
				end;
			end;
			u2 = {};
			for v127 = 1, #p58 do
				table.insert(u2, p58[v127]);
			end;
		else
			error("bad argument #2 to 'SetCollisionMode (Instance or array of Instance expected, got " .. typeof(p58) .. ")'", 2);
		end;
		u3 = nil;
	elseif p57 == v13.Function then
		if typeof(p58) ~= "function" then
			error("bad argument #2 to 'SetCollisionMode' (function expected, got " .. typeof(p58) .. ")", 2);
		end;
		u2 = nil;
		u3 = p58;
	else
		error("bad argument #1 to 'SetCollisionMode (Rain.CollisionMode expected, got " .. typeof(p58) .. ")'", 2);
	end;
	u21 = p57;
	u5 = v37[p57];
end;
