-- Script Hash: 7f905e9b5e1ad730ece33a086bdf5a747b9e06938e0189df15bd937cf4f4684f01a15923d69b88c3293b2222bd432a4c
...
local function u9(p4, p5)
	...
	if p5:FindFirstChild("RF") then
		for v3, v4 in ipairs(p5.RF:GetChildren()) do
	...
	if p5:FindFirstChild("RE") then
		for v5, v6 in ipairs(p5.RE:GetChildren()) do
			if v6:IsA("RemoteEvent") then
				v2[v6.Name] = u3.new(v6);
			end;
		end;
	end;
	if p5:FindFirstChild("RP") then
		for v7, v8 in ipairs(p5.RP:GetChildren()) do
			if v8:IsA("ValueBase") or v8:IsA("RemoteEvent") then
				v2[v8.Name] = u4.new(v8);
			end;
		end;
	end;
	u5[p4] = v2;
...
function v1.GetService(p9)
	...
	local v9 = l__Services__8:FindFirstChild(p9);
	assert(v9 ~= nil, "Could not find service \"" .. p9 .. "\"");
	return u5[p9] or u9(p9, v9);
end;
...
function v1.Start()
	...
	return u2.new(function(p11)
		local v10 = {};
		for v11, v12 in pairs(l__Controllers__15) do
			if type(v12.KnitInit) == "function" then
				table.insert(v10, u2.new(function(p12)
					debug.setmemorycategory(v12.Name);
					v12:KnitInit();
					p12();
				end));
			end;
		...
		for v13, v14 in pairs(l__Controllers__15) do
			if type(v14.KnitStart) == "function" then
				task.spawn(function()
					debug.setmemorycategory(v14.Name);
					v14:KnitStart();
				end);
			end;
		end;
