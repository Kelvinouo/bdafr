...
function v4.constructor(p1, p2, p3, p4, p5)
	p1.partyService = p2;
	p1.notificationService = p3;
	p1.queueService = p4;
	p1.lobbyServerService = p5;
end;
function v4.onInit(p6)
	p6:connectEvents();
end;
...
function v4.connectEvents(p7)
	l__LobbyServerEvents__1.joinQueue:connect(function(p8, p9)
		local v6 = p7.partyService:getPartyForPlayer(p8);
		if not v6 then
		...
		if v6:getPlayerRole(p8) ~= l__PartyRole__2.LEADER then
			p7.notificationService:sendErrorNotification({ p8 }, {
				message = "Only party leaders can join the queue."
			});
			return false;
		end;
		if v6:getQueueData() ~= nil then
			p7.notificationService:sendErrorNotification({ p8 }, {
				message = "You are already in a queue. Please leave the existing queue to join a new one."
			});
			return false;
		end;
		local v7 = p7.lobbyServerService:getQueueMeta(p9.queueType);
		if not (v7.queueSize.max < #v6:getPlayers()) then
			return ({ p7.queueService:joinQueue(v6, p9.queueType):await() })[1];
		end;
		p7.notificationService:sendErrorNotification({ p8 }, {
			message = "Max party size is " .. tostring(v7.queueSize.max) .. "."
		});
		return false;
	end);
	l__LobbyServerEvents__1.leaveQueue:connect(function(p10)
		local v8 = p7.partyService:getPartyForPlayer(p10);
		if not v8 then
			return false;
		end;
		p7.queueService:leaveQueue(v8);
		if #v8:getPlayers() > 1 then
			p7.notificationService:sendErrorNotification(v8:getPlayers(), {
...
l__Reflect__2.defineMetadata(v4, "identifier", "@easy-games/lobby:server/services/event-handlers/queue-event-handler-service@QueueEventHandlerService");
l__Reflect__2.defineMetadata(v4, "flamework:isExternal", true);
l__Reflect__2.defineMetadata(v4, "flamework:dependencies", { "@easy-games/lobby:server/services/party-service@PartyService", "@easy-games/game-core:server/services/notification-service@NotificationService", "@easy-games/lobby:server/services/queue-service@QueueService", "@easy-games/lobby:server/services/lobby-server-service@LobbyServerService" });
