...
function v3.getDto(p21)
	...
	local v17, v18, v19 = ipairs(l__members__15);
	while true do
		v17(v18, v19);
		if not v17 then
			break;
...
function v3.getSkillRating(p29, p30)
	local v20 = l__Flamework__9.resolveDependency("@easy-games/lobby:server/services/lobby-server-service@LobbyServerService"):getQueueMeta(p30);
	if not v20.skillResolver then
		return 0;
	end;
	return v20.skillResolver(p29:getPlayers());
end;
local l__DeviceType__10 = v2.DeviceType;
function v3.isMobileOnly(p31)
	local v21, v22, v23 = ipairs(p31:getPlayers());
	while true do
		v21(v22, v23);
		if not v21 then
			break;
		...
		v23 = v21;
		local v24 = l__Flamework__9.resolveDependency("@easy-games/lobby:server/services/device-info-service@DeviceInfoService"):getDeviceInfo(v22.UserId);
		if not v24 then
			return false;
		end;
		if v24.device ~= l__DeviceType__10.Mobile then
			return false;
		end;	
	end;
	return true;
