...
function v4.connectEvents(p5)
	l__LobbyServerEvents__1.lobbyPlayerConnect:connect(function(p6, p7)
		...
		if p7.party then
			local v6 = p7.party.leaderUserId;
			if v6 ~= 0 and v6 == v6 and v6 then
				v6 = p7.party.leaderUserId ~= p6.UserId;
			end;
			if v6 ~= 0 and v6 == v6 and v6 then
				local v7 = p5.partyService:getPartyByLeader(p7.party.leaderUserId);
				if v7 then
					p5.partyService:joinParty(p6, v7);
					return;
				end;
	...
	l__LobbyServerEvents__1.inviteToParty:connect(function(p8, p9)
		local v8 = p5.partyService:getPartyForPlayer(p8);
		if not v8 or v8:getPlayerRole(p8) ~= l__PartyRole__2.LEADER or table.find(v8:getPlayers(), p9.player) ~= nil then
			return false;
		end;
		v8:invitePlayer(p9.player, l__OfflinePlayerUtil__3.getOfflinePlayer(p8));
		v8:sendPartyInfoEvent();
		return true;
	end);
	l__LobbyServerEvents__1.removeFromParty:connect(function(p10, p11)
		local v9 = p5.partyService:getPartyForPlayer(p10);
		if not v9 or v9:getPlayerRole(p10) ~= l__PartyRole__2.LEADER or table.find(v9:getMembers(), p11.player) == nil then
			return false;
		end;
		p5.partyService:removePlayerFromParty(p11.player, v9, true);
	...
	l__LobbyServerEvents__1.joinParty:connect(function(p12, p13)
		local v10 = p5.partyService:getPartyForPlayer(p13.leader);
		if not v10 or not v10:isInvited(p12) then
			return false;
		end;
		return p5.partyService:joinParty(p12, v10);
	end);
	l__LobbyServerEvents__1.leaveParty:connect(function(p14)
		local v11 = p5.partyService:getPartyForPlayer(p14);
		if not v11 then
			return false;
		end;
		p5.partyService:removePlayerFromParty(p14, v11, true);
		v11:sendPartyInfoEvent();
		p5.notificationService:sendErrorNotification(v11:getPlayers(), {
	...
	l__LobbyServerEvents__1.requestPartyInfo:connect(function(p15)
		local v12 = p5.partyService:getPartyForPlayer(p15);
		if v12 then
...
l__Reflect__2.defineMetadata(v4, "identifier", "@easy-games/lobby:server/services/event-handlers/party-event-handler-service@PartyServerEventHandlerService");
l__Reflect__2.defineMetadata(v4, "flamework:isExternal", true);
l__Reflect__2.defineMetadata(v4, "flamework:dependencies", { "@easy-games/lobby:server/services/party-service@PartyService", "@easy-games/game-core:server/services/notification-service@NotificationService" });
...
return {
	PartyServerEventHandlerService = v4
};
