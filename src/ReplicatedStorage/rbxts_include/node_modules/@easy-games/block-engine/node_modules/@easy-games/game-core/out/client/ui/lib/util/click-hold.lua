...
function v3.hideProgress(p6)
	for v8, v9 in ipairs(p6.tweens) do
		local v10 = v9:Cancel();
	end;
	p6.tweens = {};
	if p6.handle then
...
function v3.endClick(p7)
	p7:hideProgress();
	if p7.startedClickTime > 0 then
		if p7.durationSeconds <= tick() - p7.startedClickTime and p7.onComplete ~= nil then
			p7.onComplete();
		end;
		if p7.onPartialComplete ~= nil then
			print("started: " .. tostring(p7.startedClickTime));
			print("duration: " .. tostring(p7.durationSeconds));
			p7.onPartialComplete((tick() - p7.startedClickTime) / p7.durationSeconds);
		end;
	end;
	p7.startedClickTime = -1;
end;
function v3.cancelClick(p8)
	p8.startedClickTime = -1;
	p8:hideProgress();
end;
function v3.disable(p9)
	p9.onComplete = nil;
	p9.onPartialComplete = nil;
	p9.startedClickTime = -1;
	p9:hideProgress();
end;
function v3.setOnComplete(p10, p11)
	p10.onComplete = p11;
end;
function v3.setOnPartialComplete(p12, p13)
	p12.onPartialComplete = p13;
end;
return {
	ClickHold = v3
};
