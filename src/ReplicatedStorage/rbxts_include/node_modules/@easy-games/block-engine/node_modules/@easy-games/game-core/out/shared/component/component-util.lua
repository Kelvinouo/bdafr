...
function u2.addComponent(p13, p14, p15, p16)
	...
	if p16 ~= 0 and p16 == p16 and p16 ~= "" and p16 then
		for v12, v13 in ipairs(u10.entries(p16)) do
...
function u2.getAllComponents(p17, p18)
	local v14 = {};
	for v15, v16 in ipairs(l__CollectionService__1:GetTagged(p18:getTag())) do
		local v17 = p17:getComponent(v16, p18);
		if v17 then
...
function u2.getComponent(p22, p23, p24)
	...
	local v18 = l__Flamework__9.resolveDependency("$c:init@Components"):getComponent(p23, p24);
	if not v18 then
		return;
	end;
	return v18;
...
function u2.getComponentFromDescendant(p25, p26, p27)
	...
	local v19 = l__Flamework__9.resolveDependency("$c:init@Components"):getComponent(p26, p27);
	if v19 then
		return v19;
	end;
...
function u2.componentAdded(p28, p29, p30)
	return l__CollectionService__1:GetInstanceAddedSignal(p29:getTag()):Connect(function(p31)
		local v20 = p28:waitForComponent(p31, p29);
		if v20 then
			p30(v20);
		end;
	end);
...
function u2.findFirstChild(p32, p33, p34)
	for v21, v22 in ipairs(p33:GetChildren()) do
		local v23 = u2:getComponent(v22, p34);
		if v23 then
			return v23;
		end;
	end;
...
function u2.findFirstDescendant(p35, p36, p37)
	for v24, v25 in ipairs(p36:GetDescendants()) do
		local v26 = u2:getComponent(v25, p37);
		if v26 then
			return v26;
		end;
	end;
...
return {
	WatchComponent = function(p38, p39)
		for v27, v28 in ipairs(u2:getAllComponents(p38)) do
			task.spawn(function()
				p39(v28);
			end);
		end;
	...
	WatchInstanceComponent = function(p41, p42, p43)
		local v29 = u3.new();
		local v30 = u2:getComponent(p41, p42);
		if v30 then
			task.spawn(function()
				p43(v30);
			end);
		end;
		local v31 = l__MapUtil__4.getOrCreate(u5, p42, {});
		local v32 = v31[p41];
		if not v32 then
			v32 = {};
			v31[p41] = v32;
		...
		table.insert(v32, p43);
		v29:GiveTask(function()
			local v33 = v32;
			if v33 ~= nil then
				v33 = (table.find(v33, p43) and 0) - 1;
			end;
