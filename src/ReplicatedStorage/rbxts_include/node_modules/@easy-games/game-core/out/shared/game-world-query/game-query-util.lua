-- Decompiled with the Synapse X Luau decompiler.

local v1 = setmetatable({}, {
	__tostring = function()
		return "GameQueryUtil";
	end
});
v1.__index = v1;
function v1.new(...)
	local v2 = setmetatable({}, v1);
	return v2:constructor(...) and v2;
end;
function v1.constructor(p1)

end;
function v1.isQueryIgnored(p2, p3)
	if not p3:IsA("BasePart") then
		return false;
	end;
	return p3:GetAttribute("gamecore_GameQueryIgnore") == true;
end;
function v1.setQueryIgnored(p4, p5, p6)
	if p6 == nil then
		p6 = true;
	end;
	if not p5:IsA("BasePart") then
		return nil;
	end;
	if p6 then
		local v3 = true;
	else
		v3 = nil;
	end;
	p5:SetAttribute("gamecore_GameQueryIgnore", v3);
	if p4.ADJUST_CAN_QUERY then
		p5.CanQuery = not p6;
	end;
end;
function v1.raycast(p7, p8, p9, p10, p11)
	local v4 = nil;
	debug.profilebegin("game-query-raycast");
	if not p10 then
		p10 = RaycastParams.new();
	end;
	local v5 = {};
	local v6 = p10.FilterDescendantsInstances;
	table.move(v6, 1, #v6, #v5 + 1, v5);
	while true do
		local v7 = {};
		table.move(v5, 1, #v5, #v7 + 1, v7);
		p10.FilterDescendantsInstances = v7;
		local v8 = p11;
		if v8 ~= nil then
			v8 = v8.world;
		end;
		local v9 = v8;
		if v9 == nil then
			v9 = game.Workspace;
		end;
		v4 = v9:Raycast(p8, p9, p10);
		if v4 then
			if not p7:isQueryIgnored(v4.Instance) then
				local v10 = p11;
				if v10 ~= nil then
					v10 = v10.ignorePart;
				end;
				if not v10 or not p11.ignorePart(v4.Instance) then
					break;
				end;
			end;
			if p10.FilterType == Enum.RaycastFilterType.Blacklist then
				table.insert(v5, v4.Instance);
			else
				local v11 = -1;
				local v12, v13, v14 = ipairs(p10.FilterDescendantsInstances);
				while true do
					v12(v13, v14);
					if not v12 then
						break;
					end;
					local v15 = v4;
					if v15 ~= nil then
						v15 = v15.Instance:IsDescendantOf(v13);
					end;
					local v16 = v15;
					if not v16 then
						local v17 = v4;
						if v17 ~= nil then
							v17 = v17.Instance;
						end;
						v16 = v17 == v13;
					end;
					if v16 == true then
						v11 = v12 - 1;
						break;
					end;				
				end;
				table.remove(v5, v11 + 1);
			end;
		end;
		if v4 == nil then
			break;
		end;	
	end;
	debug.profileend();
	return v4;
end;
v1.ADJUST_CAN_QUERY = false;
return {
	GameQueryUtil = v1
};
