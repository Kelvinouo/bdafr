...
function v2.getCode(p7, p8)
	local v15 = nil;
	for v16, v17 in ipairs(p7.gameCodes) do
	...
	local v18 = p7.codesDataStore;
	if v18 ~= nil then
		v18 = v18:GetAsync(string.upper(p8));
	end;
	if v18 then
		return v18;
	end;
...
function v2.addGeneratedCodes(p18, p19)
	for v19, v20 in ipairs(p19) do
		local l__codesDataStore__21 = p18.codesDataStore;
		if l__codesDataStore__21 ~= nil then
			l__codesDataStore__21:SetAsync(v20.code, v20);
		end;
	end;
end;
...
function v2.getPlayerCodeGenInfo(p23, p24)
	local v22 = u5.entries((p23:getCodeGenerators()));
	local u6 = {};
	local function v23(p25)
		local v24 = p25[2];
		if not v24:hasPermission(p24.UserId) then
			return nil;
		end;
		u6[p25[1]] = {
			codes = v24:getGeneratedCodes(p24), 
			lastRegenTime = v24:getLastGenerateTime(p24)
		};
	end;
