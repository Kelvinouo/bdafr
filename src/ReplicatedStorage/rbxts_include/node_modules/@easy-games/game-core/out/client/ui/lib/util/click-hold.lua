-- Script Hash: 9b8390397b6f8cc09ab42b8e1908b2424e90f211d6c4206d83c352a1b130b76446fc1365287508667fbd76b7b1a85914
...
function v3.hideProgress(p11)
	local l__tweens__33 = p11.tweens;
	local function v34(p12)
		return p12:Cancel();
	end;
	for v35, v36 in ipairs(l__tweens__33) do
		v34(v36, v35 - 1, l__tweens__33);
	end;
	p11.tweens = {};
	if p11.handle then
...
function v3.endClick(p13)
	p13:hideProgress();
	if p13.startedClickTime > 0 then
		if p13.durationSeconds <= tick() - p13.startedClickTime and p13.onComplete ~= nil then
			p13.onComplete();
		end;
		if p13.onPartialComplete ~= nil then
			p13.onPartialComplete((tick() - p13.startedClickTime) / p13.durationSeconds);
		end;
	end;
	p13.startedClickTime = -1;
end;
function v3.cancelClick(p14)
	p14.startedClickTime = -1;
	p14:hideProgress();
end;
function v3.disable(p15)
	p15.onComplete = nil;
	p15.onPartialComplete = nil;
	p15.startedClickTime = -1;
	p15:hideProgress();
end;
function v3.setOnComplete(p16, p17)
	p16.onComplete = p17;
end;
function v3.setOnPartialComplete(p18, p19)
	p18.onPartialComplete = p19;
end;
