...
function v3.getId(p22)
	return p22.id;
end;
function v3.getIndicatorUi(p23)
	return p23.ui;
end;
function v3.getTargetPosition(p24)
	local l__attachTo__27 = p24.config.attachTo;
...
function v3.update(p25)
	local v28 = p25:getIndicatorWorldPosition();
	local v29 = v3.camera.CFrame;
	local v30 = p25.config.uiConfig.distanceRelativeToPlayer;
	if v30 then
	...
	local l__AbsolutePosition__36 = p25.boundaryUi.AbsolutePosition;
	local l__AbsoluteSize__37 = p25.boundaryUi.AbsoluteSize;
	if p25.config.constraintType == l__ConstraintType__4.Constrained then
		if l__Z__33 < 0 then
		...
		p25.ui.Position = UDim2.fromOffset(v39.X - l__AbsolutePosition__36.X + v42.X, v39.Y - l__AbsolutePosition__36.Y + v42.Y);
		if p25.arrow then
			p25.arrow.Visible = v34 ~= v39;
			if v34 ~= v39 then
				local v43 = v34 - v39;
				...
				p25.arrow.Rotation = v44;
			end;
		end;
	elseif p25.config.constraintType == l__ConstraintType__4.Hidden then
		if l__Z__33 < 0 then
		...
		elseif v3:screenBoundaryMethod(Rect.new(l__AbsolutePosition__36, l__AbsolutePosition__36 + l__AbsoluteSize__37), v34) then
			p25.ui.Visible = true;
			p25.ui.Position = UDim2.fromOffset(v34.X - l__AbsolutePosition__36.X, v34.Y - l__AbsolutePosition__36.Y);
		else
			p25.ui.Visible = false;
		end;
		if p25.arrow then
			p25.arrow.Visible = false;
		end;
	elseif p25.config.constraintType == l__ConstraintType__4.Radial then
		...
		p25.ui.Position = UDim2.fromOffset(v48.X - l__AbsolutePosition__36.X + v51.X, v48.Y - l__AbsolutePosition__36.Y + v51.Y);
		if p25.arrow then
			p25.arrow.Visible = true;
			p25.arrow.Rotation = math.deg(math.atan2(v46.Y, v46.X));
		end;
	end;
	...
	if p25.config.uiConfig.easeOutOnAppoach == nil then
		local v54 = true;
	else
		v54 = p25.config.uiConfig.easeOutOnAppoach;
	end;
	if v54 then
		local v55 = v52;
	else
	...
	p25:updateProperties(l__Magnitude__35, v55, v53);
	if p25.config.onUpdateProperties then
		if v54 then
			local v56 = v52;
		else
			v56 = 0;
		end;
		p25.config.onUpdateProperties(l__Magnitude__35, v56, v53, p25.ui, p25.config.creator);
	end;
end;
function v3.getIndicatorWorldPosition(p26)
	local l__attachTo__57 = p26.config.attachTo;
...
function v3.createUi(p27)
	local v58 = u6("Frame", {
		AnchorPoint = Vector2.new(0.5, 0.5), 
		Size = v3.BASE_INDICATOR_SIZE, 
		BackgroundTransparency = 1, 
		Parent = p27.boundaryUi, 
	...
	if type(p27.config.uiConfig.color3) == "function" then
		local v59 = p27.config.uiConfig.color3(0);
	else
		v59 = p27.config.uiConfig.color3;
	end;
	local v60 = {
		AnchorPoint = Vector2.new(0.5, 0.5), 
	...
	local v62 = p27.config.arrowImage;
	if v62 == nil then
	...
	p27.arrow = u6("Frame", v60);
	return p27:mountUi(v58);
end;
function v3.getDirection(p28, p29)
	local v63 = p29 or v3.camera.CFrame.Position;
	local v64 = p28.config.uiConfig.distanceRelativeToPlayer;
	if v64 then
		local v65 = l__Players__3.LocalPlayer.Character;
		if v65 ~= nil then
			v65 = v65.PrimaryPart;
		end;
		v64 = v65 ~= nil;
	...
	if v64 then
		v63 = l__Players__3.LocalPlayer.Character.PrimaryPart.CFrame.Position;
	end;
	return (p28:getIndicatorWorldPosition() - v63).Unit;
end;
function v3.destroy(p30)
	p30.maid:Destroy();
end;
function v3.Destroy(p31)
	p31:destroy();
end;
