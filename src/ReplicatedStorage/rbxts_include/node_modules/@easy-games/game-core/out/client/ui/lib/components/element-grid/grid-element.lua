...
local l__ColorUtil__6 = v1.import(script, script.Parent.Parent.Parent.Parent.Parent.Parent, "shared", "util", "color-util").ColorUtil;
return {
	GridElement = v1.import(script, v1.getModule(script, "@rbxts", "roact-hooks").src).new(u4)(function(p1, p2)
		...
		l__useEffect__4(function()
			v6(p1.CurrentIndex == p1.Index);
			if p1.CurrentIndex == p1.Index then
				local v15 = 1;
			else
				v15 = 0;
			end;
			v7:setGoal(l__Spring__2.new(v15, {
				frequency = 8, 
				dampingRatio = 1
			}));
		end, { p1.CurrentIndex });
		local v16 = {};
		local l__TextElement__17 = p1.TextElement;
		...
		v20[u4.Event.Activated] = function()
			p1.OnClick();
			if p1.SetSelectedIndex then
				p1.SetSelectedIndex(p1.Index);
			end;
		end;
		local function u7(p3)
			v11:setGoal(l__Spring__2.new(p3, {
				frequency = 9, 
				dampingRatio = 0.9
			}));
		end;
		...
		local v25 = v13;
		if v25 ~= nil then
			v25 = v25:map(function(p4)
				return 1 + -0.09999999999999998 * ((p4 - 0) / 1);
			end);
		end;
		v24.Scale = v25;
		...
		local v30 = v9;
		if v30 ~= nil then
			v30 = v30:map(function(p5)
				return NumberSequence.new({ NumberSequenceKeypoint.new(0, 0.1 + 0.4 * ((p5 - 0) / 1)), NumberSequenceKeypoint.new(0.35, 1 + -0.5 * ((p5 - 0) / 1)), NumberSequenceKeypoint.new(1, 1 + -0.5 * ((p5 - 0) / 1)) });
			end);
		end;
		...
		if v31 then
			...
			local v33 = v9;
			if v33 ~= nil then
				v33 = v33:map(function(p6)
					return 1 + -1 * ((p6 - 0) / 1);
				end);
			end;
			v32.ImageTransparency = v33;
			local v34 = p1.ActiveIcon;
			if v34 == nil then
				v34 = u5.Image;
			end;
			v32.Image = v34;
			local l__ActiveIcon__35 = p1.ActiveIcon;
			if l__ActiveIcon__35 == "" or not l__ActiveIcon__35 then
				local v36 = u5.ImageRectOffset;
			else
				v36 = nil;
			end;
			v32.ImageRectOffset = v36;
			local l__ActiveIcon__37 = p1.ActiveIcon;
			if l__ActiveIcon__37 == "" or not l__ActiveIcon__37 then
				local v38 = u5.ImageRectSize;
			else
				v38 = nil;
			end;
		...
		if v39 then
			local v40 = {
				...
				BackgroundTransparency = 1, 
				Text = "<b>" .. tostring(p1.TextElement.Text) .. "</b>", 
				TextWrapped = true, 
				RichText = true, 
