...
return {
	...
	Linear = function(p17, p18, p19, p20)
		return p19 * p17 / p20 + p18;
	end, 
	InSine = function(p21, p22, p23, p24)
		return -p23 * l__math_cos__2(p21 / p24 * 1.5707963267948966) + p23 + p22;
	end, 
	OutSine = function(p25, p26, p27, p28)
		return p27 * l__math_sin__1(p25 / p28 * 1.5707963267948966) + p26;
	end, 
	InOutSine = function(p29, p30, p31, p32)
		return -p31 * 0.5 * (l__math_cos__2(3.141592653589793 * p29 / p32) - 1) + p30;
	end, 
	OutInSine = function(p33, p34, p35, p36)
		p35 = p35 * 0.5;
		if p33 < p36 * 0.5 then
			return p35 * l__math_sin__1(p33 * 2 / p36 * 1.5707963267948966) + p34;
		end;
		return -p35 * l__math_cos__2((p33 * 2 - p36) / p36 * 1.5707963267948966) + 2 * p35 + p34;
	end, 
	InBack = function(p37, p38, p39, p40, p41)
		p41 = p41 and 1.70158;
		p37 = p37 / p40;
		return p39 * p37 * p37 * ((p41 + 1) * p37 - p41) + p38;
	end, 
	OutBack = function(p42, p43, p44, p45, p46)
		p46 = p46 and 1.70158;
		p42 = p42 / p45 - 1;
		return p44 * (p42 * p42 * ((p46 + 1) * p42 + p46) + 1) + p43;
	end, 
	InOutBack = function(p47, p48, p49, p50, p51)
		p51 = (p51 and 1.70158) * 1.525;
		p47 = p47 / p50 * 2;
		if p47 < 1 then
			return p49 * 0.5 * (p47 * p47 * ((p51 + 1) * p47 - p51)) + p48;
		end;
		p47 = p47 - 2;
		return p49 * 0.5 * (p47 * p47 * ((p51 + 1) * p47 + p51) + 2) + p48;
	end, 
	OutInBack = function(p52, p53, p54, p55, p56)
		p54 = p54 * 0.5;
		p56 = p56 and 1.70158;
		if p52 < p55 * 0.5 then
			p52 = p52 * 2 / p55 - 1;
			return p54 * (p52 * p52 * ((p56 + 1) * p52 + p56) + 1) + p53;
		end;
		p52 = (p52 * 2 - p55) / p55;
		return p54 * p52 * p52 * ((p56 + 1) * p52 - p56) + p53 + p54;
	end, 
	InQuad = function(p57, p58, p59, p60)
		p57 = p57 / p60;
		return p59 * p57 * p57 + p58;
	end, 
	OutQuad = function(p61, p62, p63, p64)
		p61 = p61 / p64;
		return -p63 * p61 * (p61 - 2) + p62;
	end, 
	InOutQuad = function(p65, p66, p67, p68)
		p65 = p65 / p68 * 2;
		if p65 < 1 then
			return p67 * 0.5 * p65 * p65 + p66;
		end;
		return -p67 * 0.5 * ((p65 - 1) * (p65 - 3) - 1) + p66;
	end, 
	OutInQuad = function(p69, p70, p71, p72)
		if p69 < p72 * 0.5 then
			p69 = 2 * p69 / p72;
			return -0.5 * p71 * p69 * (p69 - 2) + p70;
		end;
		p69 = (p69 * 2 - p72) / p72;
		p71 = 0.5 * p71;
		return p71 * p69 * p69 + p70 + p71;
	end, 
	InQuart = function(p73, p74, p75, p76)
		p73 = p73 / p76;
		return p75 * p73 * p73 * p73 * p73 + p74;
	end, 
	OutQuart = function(p77, p78, p79, p80)
		p77 = p77 / p80 - 1;
		return -p79 * (p77 * p77 * p77 * p77 - 1) + p78;
	end, 
	InOutQuart = function(p81, p82, p83, p84)
		p81 = p81 / p84 * 2;
		if p81 < 1 then
			return p83 * 0.5 * p81 * p81 * p81 * p81 + p82;
		end;
		p81 = p81 - 2;
		return -p83 * 0.5 * (p81 * p81 * p81 * p81 - 2) + p82;
	end, 
	OutInQuart = function(p85, p86, p87, p88)
		if not (p85 < p88 * 0.5) then
			p85 = (p85 * 2 - p88) / p88;
			p87 = p87 * 0.5;
			return p87 * p85 * p85 * p85 * p85 + p86 + p87;
		end;
		p85 = p85 * 2 / p88 - 1;
		p87 = p87 * 0.5;
		return -p87 * (p85 * p85 * p85 * p85 - 1) + p86;
	end, 
	InQuint = function(p89, p90, p91, p92)
		p89 = p89 / p92;
		return p91 * p89 * p89 * p89 * p89 * p89 + p90;
	end, 
	OutQuint = function(p93, p94, p95, p96)
		p93 = p93 / p96 - 1;
		return p95 * (p93 * p93 * p93 * p93 * p93 + 1) + p94;
	end, 
	InOutQuint = function(p97, p98, p99, p100)
		p97 = p97 / p100 * 2;
		if p97 < 1 then
			return p99 * 0.5 * p97 * p97 * p97 * p97 * p97 + p98;
		end;
		p97 = p97 - 2;
		return p99 * 0.5 * (p97 * p97 * p97 * p97 * p97 + 2) + p98;
	end, 
	OutInQuint = function(p101, p102, p103, p104)
		if p101 < p104 * 0.5 then
			p101 = p101 * 2 / p104 - 1;
			return p103 * 0.5 * (p101 * p101 * p101 * p101 * p101 + 1) + p102;
		end;
		p101 = (p101 * 2 - p104) / p104;
		p103 = p103 * 0.5;
		return p103 * p101 * p101 * p101 * p101 * p101 + p102 + p103;
	end, 
	InBounce = function(p105, p106, p107, p108)
		return p107 - u7(p108 - p105, 0, p107, p108) + p106;
	end, 
	OutBounce = u7, 
	InOutBounce = function(p109, p110, p111, p112)
		if p109 < p112 * 0.5 then
			return (p111 - u7(p112 - p109 * 2, 0, p111, p112) + 0) * 0.5 + p110;
		end;
	...
	OutInBounce = function(p113, p114, p115, p116)
		...
		local v4 = p115 * 0.5;
		return v4 - u7(p116 - (p113 * 2 - p116), 0, v4, p116) + (p114 + p115 * 0.5);
	end, 
	InElastic = u6, 
	OutElastic = u5, 
	InOutElastic = function(p117, p118, p119, p120, p121, p122)
		...
		if p121 == nil or p121 < v5 then
			p121 = p119;
			local v6 = p122 * 0.25;
		else
			v6 = p122 / 6.283185307179586 * l__math_asin__4(p119 / p121);
		end;
		if p117 < 1 then
			return -0.5 * p121 * 1024 ^ p117 * l__math_sin__1((p117 * p120 - v6) * 6.283185307179586 / p122) + p118;
		end;
		return p121 * l__math_exp__3(-6.931471805599453 * p117) * l__math_sin__1((p117 * p120 - v6) * 6.283185307179586 / p122) * 0.5 + p119 + p118;
	end, 
