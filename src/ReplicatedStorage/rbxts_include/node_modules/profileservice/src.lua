-- Script Hash: dac84210d593a1d2234f0f14b620919ec0cdba02c99dec59c84d3f654f63739cb6eefb722d4e00b861f460c8d4d352e9
...
local function u42(p81, p82, p83)
	...
	while v117 == true do
		...
		if v135 ~= nil and v136 ~= nil then
			...
			for v142 in pairs(u7.MetaTagsUpdatedValues) do
				l__MetaData__140[v142] = l__MetaData__141[v142];
			end;
			l__MetaData__140.MetaTagsLatest = l__MetaData__141.MetaTags;
			local l__ActiveSession__143 = v135.MetaData.ActiveSession;
			local v144 = false;
			if type(l__ActiveSession__143) == "table" then
				v144 = u36(l__ActiveSession__143) and v135.MetaData.SessionLoadCount == l__SessionLoadCount__116;
			end;
			v137 = p81:IsActive();
			if v144 == true then
				if v137 == true then
...
function v109.RemoveUserId(p91, p92)
	...
	local v145 = table.find(p91.UserIds, p92);
	if v145 ~= nil then
		table.remove(p91.UserIds, v145);
	end;
end;
...
function v109.ClearGlobalUpdates(p94)
	...
	setmetatable(v146, v74);
	p94.GlobalUpdates = v146;
end;
...
function v147.NextAsync(p97, p98)
	local v148 = nil;
	...
	v148 = function()
		...
		if p97._query_pages == nil then
			...
			if v149 == false or p97._query_pages == nil then
				warn("[ProfileService]: Version query fail - " .. tostring(v150));
		...
		local v151 = p97._query_pages:GetCurrentPage()[p97._query_index + 1];
		if p97._query_pages.IsFinished == true and v151 == nil then
			u43 = true;
			return;
		end;
		if v151 ~= nil then
			p97._query_index = p97._query_index + 1;
			u44 = p97._profile_store:ViewProfileAsync(p97._profile_key, v151.Version);
			u43 = true;
			return;
		end;
		p97._is_query_yielded = true;
	...
	elseif p98 == true then
		table.insert(p97._query_queue, 1, v148);
	else
		table.insert(p97._query_queue, v148);
	end;
	while u43 == false do
		task.wait();	
	end;
...
local u47 = 0;
local u48 = 0;
function v152.LoadProfileAsync(p100, p101, p102, p103)
	...
	u45(p100);
	local v153 = p103 == u46;
	for v154, v155 in ipairs(l___active_profile_stores__27) do
		if v155._profile_store_lookup == p100._profile_store_lookup and (v153 == true and v155._mock_loaded_profiles or v155._loaded_profiles)[p101] ~= nil then
	...
	u47 = u47 + 1;
	local v156 = p102 == "ForceLoad";
	local v157 = 0;
	local v158 = false;
	local v159 = p102 == "Steal";
	while v10.ServiceLocked == false do
		local v160 = v153 == true and p100._mock_profile_load_jobs or p100._profile_load_jobs;
		local v161 = u48 + 1;
		u48 = v161;
		local v162 = v160[p101];
		if v162 ~= nil then
			v162[1] = v161;
			while v162[2] == nil do
				task.wait();			
			...
			local v163, v164 = table.unpack(v162[2]);
			local v165 = v163;
			local v166 = v164;
			v160[p101] = nil;
		else
			...
			v160[p101] = v167;
			local v168 = {};
			function v168.ExistingProfileHandle(p104)
				if v10.ServiceLocked == false then
					local l__ActiveSession__169 = p104.MetaData.ActiveSession;
					local l__ForceLoadSession__170 = p104.MetaData.ForceLoadSession;
					if l__ActiveSession__169 == nil then
						p104.MetaData.ActiveSession = { l__PlaceId__15, l__JobId__16 };
						p104.MetaData.ForceLoadSession = nil;
						return;
					end;
					if type(l__ActiveSession__169) == "table" then
						if u36(l__ActiveSession__169) == false then
							local l__LastUpdate__171 = p104.MetaData.LastUpdate;
							if l__LastUpdate__171 ~= nil and u7.AssumeDeadSessionLock < os.time() - l__LastUpdate__171 then
								p104.MetaData.ActiveSession = { l__PlaceId__15, l__JobId__16 };
								p104.MetaData.ForceLoadSession = nil;
								return;
							end;
							if v158 == true or v159 == true then
								local v172 = false;
								if l__ForceLoadSession__170 ~= nil then
									v172 = u36(l__ForceLoadSession__170);
								end;
								if v172 == true or v159 == true then
									p104.MetaData.ActiveSession = { l__PlaceId__15, l__JobId__16 };
									p104.MetaData.ForceLoadSession = nil;
									return;
								end;
							elseif v156 == true then
			...
			function v168.MissingProfileHandle(p105)
			...
			function v168.EditProfile(p106)
				if v10.ServiceLocked == false then
					local l__ActiveSession__173 = p106.MetaData.ActiveSession;
					if l__ActiveSession__173 ~= nil and u36(l__ActiveSession__173) == true then
						p106.MetaData.SessionLoadCount = p106.MetaData.SessionLoadCount + 1;
						p106.MetaData.LastUpdate = os.time();
					end;
				end;
			end;
			v167[2] = table.pack(u31(p100, p101, v168, v153));
			if v167[1] ~= v161 then
				u47 = u47 - 1;
				return nil;
			end;
			local v174, v175 = table.unpack(v167[2]);
			v165 = v174;
			v166 = v175;
			v160[p101] = nil;
		end;
		if v165 ~= nil and v166 ~= nil then
			local l__ActiveSession__176 = v165.MetaData.ActiveSession;
			if type(l__ActiveSession__176) ~= "table" then
				u47 = u47 - 1;
				return nil;
			end;
			if u36(l__ActiveSession__176) == true then
				v165.MetaData.MetaTagsLatest = u4(v165.MetaData.MetaTags);
				local v177 = {
					_updates_latest = v165.GlobalUpdates, 
				...
				setmetatable(v177, v74);
				local v178 = {
					Data = v165.Data, 
					MetaData = v165.MetaData, 
					MetaTagsUpdated = v9.NewScriptSignal(), 
					RobloxMetaData = v165.RobloxMetaData or {}, 
					UserIds = v165.UserIds or {}, 
					KeyInfo = v166, 
					KeyInfoUpdated = v9.NewScriptSignal(), 
					GlobalUpdates = v177, 
					_profile_store = local v179, 
					_profile_key = p101, 
				...
				setmetatable(v178, v109);
				v177._profile = v178;
				if next(v179._loaded_profiles) == nil and next(v179._mock_loaded_profiles) == nil then
					table.insert(l___active_profile_stores__27, v179);
				end;
				if v153 == true then
					v179._mock_loaded_profiles[p101] = v178;
				else
					v179._loaded_profiles[p101] = v178;
				end;
				u41(v178);
				if v10.ServiceLocked == true then
					u42(v178, true);
			...
			if v156 == true then
				local l__ForceLoadSession__180 = v165.MetaData.ForceLoadSession;
				local v181 = false;
				if l__ForceLoadSession__180 ~= nil then
					v181 = u36(l__ForceLoadSession__180);
				end;
				if v181 ~= true then
					u47 = u47 - 1;
					return nil;
				end;
				if local v182 == false then
					v157 = v157 + 1;
					if v157 == u7.ForceLoadMaxSteps then

					end;
				end;
				task.wait();
				v182 = false;
			elseif local v183 == true then
				task.wait();
			else
				local v184 = p102(l__ActiveSession__176[1], l__ActiveSession__176[2]);
				if v184 == "Repeat" then
					task.wait();
				else
					if v184 == "Cancel" then
					...
					if v184 == "ForceLoad" then
						v156 = true;
						v182 = true;
						task.wait();
					elseif v184 == "Steal" then
						task.wait();
					else
						error("[ProfileService]: Invalid return from not_released_handler (\"" .. tostring(v184) .. "\")(" .. type(v184) .. ");" .. "\n" .. u10(v179._profile_store_name, v179._profile_store_scope, p101) .. " Traceback:\n" .. debug.traceback());
					end;
				end;
			end;
		else
...
function v152.GlobalUpdateProfileAsync(p107, p108, p109, p110)
	...
	while v10.ServiceLocked == false do
		...
		function v185.EditProfile(p111)
			local v186 = {
				_updates_latest = p111.GlobalUpdates, 
				_update_handler_mode = true
			};
			setmetatable(v186, v74);
			p109(v186);
		end;
		local v187 = u31(p107, p108, v185, p110 == u46);
		u38(p107._profile_store_lookup, p108);
		if v187 ~= nil then
			local v188 = {
				_updates_latest = v187.GlobalUpdates
			};
			setmetatable(v188, v74);
			return v188;
		end;
		task.wait();	
	end;
...
function v152.ViewProfileAsync(p112, p113, p114, p115)
	...
	while v10.ServiceLocked == false do
		...
		function v189.MissingProfileHandle(p116)
		...
		v189.EditProfile = nil;
		local v190, v191 = u31(p112, p113, v189, p115 == u46, true, p114);
		...
		if v190 ~= nil then
			if v191 == nil then
				...
				local v192 = {
					_updates_latest = v190.GlobalUpdates, 
					_profile = nil
				};
				setmetatable(v192, v74);
				local v193 = {
					Data = v190.Data, 
					MetaData = v190.MetaData, 
					MetaTagsUpdated = v9.NewScriptSignal(), 
					RobloxMetaData = v190.RobloxMetaData or {}, 
					UserIds = v190.UserIds or {}, 
					KeyInfo = v191, 
					KeyInfoUpdated = v9.NewScriptSignal(), 
					GlobalUpdates = v192, 
					_profile_store = p112, 
...
function v152.ProfileVersionQuery(p117, p118, p119, p120, p121, p122)
	if type(p118) ~= "string" or string.len(p118) == 0 then
		error("[ProfileService]: Invalid profile_key");
	end;
	if v10.ServiceLocked == true then
		return setmetatable({}, v147);
	end;
	u45(p117);
	...
	setmetatable(v194, v147);
	return v194;
end;
function v152.WipeProfileAsync(p123, p124, p125)
	...
	u45(p123);
	if p125 == u46 then
		local v195 = l___user_mock_data_store__18[p123._profile_store_lookup];
		if v195 ~= nil then
			v195[p124] = nil;
		end;
		local v196 = true;
	...
	elseif u19 == true then
		local v197 = l___mock_data_store__20[p123._profile_store_lookup];
		if v197 ~= nil then
			v197[p124] = nil;
		end;
		v196 = true;
...
function v10.GetProfileStore(p126, p127)
	...
	if v198 == nil or type(v198) ~= "string" then
		error("[ProfileService]: Missing or invalid \"Name\" parameter");
	elseif string.len(v198) == 0 then
		error("[ProfileService]: ProfileStore name cannot be an empty string");
	end;
	if v199 ~= nil and (type(v199) ~= "string" or string.len(v199) == 0) then
	...
	local v201 = {};
	local u51 = nil;
	function v201.LoadProfileAsync(p128, p129, p130)
		return u51:LoadProfileAsync(p129, p130, u46);
	end;
	function v201.GlobalUpdateProfileAsync(p131, p132, p133)
		return u51:GlobalUpdateProfileAsync(p132, p133, u46);
	end;
	function v201.ViewProfileAsync(p134, p135, p136)
		return u51:ViewProfileAsync(p135, p136, u46);
	end;
	function v201.FindProfileVersionAsync(p137, p138, p139, p140, p141)
		return u51:FindProfileVersionAsync(p138, p139, p140, p141, u46);
	end;
	function v201.WipeProfileAsync(p142, p143)
		return u51:WipeProfileAsync(p143, u46);
	end;
	v200.Mock = v201;
	v200._profile_store_name = v198;
	v200._profile_store_scope = v199;
	v200._profile_store_lookup = v198 .. "\000" .. (v199 and "");
	v200._profile_template = p127;
	v200._global_data_store = nil;
	v200._loaded_profiles = {};
	v200._profile_load_jobs = {};
	v200._mock_loaded_profiles = {};
	v200._mock_profile_load_jobs = {};
	v200._is_pending = false;
	u51 = v200;
	setmetatable(u51, v152);
	local v202 = Instance.new("DataStoreOptions");
	v202:SetExperimentalFeatures({
		v2 = true
	});
	if u9 == true then
		...
		task.spawn(function()
			u49();
			if u19 == false then
				u51._global_data_store = l__DataStoreService__50:GetDataStore(v198, v199, v202);
			end;
...
if l__RunService__11:IsStudio() == true then
	...
	task.spawn(function()
		...
		local v205 = false;
		if v203 == false then
			v205 = string.find(v204, "ConnectFail", 1, true) ~= nil;
		end;
		if v205 == true then
			warn("[ProfileService]: No internet access - check your network connection");
		end;
		if v203 == false and (string.find(v204, "403", 1, true) ~= nil or string.find(v204, "must publish", 1, true) ~= nil or v205 == true) then
			u19 = true;
...
l__RunService__11.Heartbeat:Connect(function()
	local v206 = #l___auto_save_list__24;
	if v206 > 0 then
		local v207 = u7.AutoSaveProfiles / v206;
		local v208 = os.clock();
		while v207 < v208 - u26 do
			u26 = u26 + v207;
			local v209 = l___auto_save_list__24[u25];
			if v208 - v209._load_timestamp < u7.AutoSaveProfiles then
				for v210 = 1, v206 - 1 do
					u25 = u25 + 1;
					if v206 < u25 then
						u25 = 1;
					end;
					v209 = l___auto_save_list__24[u25];
					if u7.AutoSaveProfiles <= v208 - v209._load_timestamp then
						break;
					end;
					v209 = nil;
				end;
	...
	while true do
		local v211 = l___issue_queue__11[1];
		if v211 == nil then
			return;
		end;
		if not (u7.IssueLast < os.clock() - v211) then
			break;
		end;
...
task.spawn(function()
	...
	v9.ConnectToOnClose(function()
		...
		local v212 = 0;
		local v213 = {};
		for v214, v215 in ipairs(l___auto_save_list__24) do
			v213[v214] = v215;
		end;
		for v216, v217 in ipairs(v213) do
			if v217:IsActive() == true then
				v212 = v212 + 1;
				local u53 = v212;
				task.spawn(function()
					u42(v217, true);
		...
		while v212 > 0 or u47 > 0 or u30 > 0 do
			task.wait();		
		end;
	end, u19 == false);
end);
