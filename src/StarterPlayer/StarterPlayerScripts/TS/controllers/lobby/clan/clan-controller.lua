...
function v5.KnitStart(p2)
	...
	l__default__1.Client:GetNamespace("Clans"):OnEvent("ClanUpdate", function(p3)
		...
		if p3.clanId == nil then
			l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):closeApp(l__BedwarsAppIds__3.CLAN_PROFILE);
		end;
	end);
	l__default__1.Client:GetNamespace("Clans"):OnEvent("ClanInvite", function(p4)
		l__ClientStore__6:dispatch({
		...
		l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/notification-controller@NotificationController"):sendInfoNotification({
			image = l__BedwarsImageId__7.CLAN, 
			message = "<b>Clan <font color=\"" .. l__ColorUtil__8.richTextColor(l__Theme__9.mcAqua) .. "\">[" .. p4.clanProfile.tag .. "]</font> has sent you an invite!</b>", 
			onClick = function()
				l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):openApp(l__BedwarsAppIds__3.CLAN_MENU, {});
			end
		});
	end);
	l__default__1.Client:GetNamespace("Clans"):OnEvent("OpenClanMenu", function(p5)
		if p5.open then
			l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):openApp(l__BedwarsAppIds__3.CLAN_MENU, {});
			return;
		end;
		l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):closeApp(l__BedwarsAppIds__3.CLAN_MENU);
	end);
	l__Players__10.LocalPlayer:GetAttributeChangedSignal("ClanPassPaid"):Connect(function()
		l__ClientStore__6:dispatch({
			type = "ClanSetClanPassPaid", 
			paid = l__ClanUtil__11.hasClanPass(l__Players__10.LocalPlayer)
		});
	end);
end;
...
local l__MarketplaceService__12 = v3.MarketplaceService;
local l__Gamepass__13 = v1.import(script, game:GetService("ReplicatedStorage"), "TS", "place", "gamepass").Gamepass;
function v5.promptClanPassPurchase(p9)
	if not l__ClanUtil__11.hasClanPass(l__Players__10.LocalPlayer) then
		l__MarketplaceService__12:PromptGamePassPurchase(l__Players__10.LocalPlayer, l__Gamepass__13.CLAN_CREATE_PASS);
	end;
end;
...
local l__UILayers__14 = v2.UILayers;
function v5.leaveClan(p29)
	local v9 = {
		...
		PrimaryBtnProps = {
			Text = "Leave", 
			BackgroundColor3 = l__Theme__9.backgroundError
		}, 
		SecondaryBtnProps = {
			Text = "Cancel"
		}
	...
	function v9.OnPrimaryBtnClick()
		if not p29.clansRemotes:Get("LeaveClan"):CallServer() then
			l__SoundManager__4:playSound(l__GameSound__5.ERROR_NOTIFICATION);
			return;
		end;
		l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):closeApp(l__BedwarsAppIds__3.CLAN_PROFILE);
		l__SoundManager__4:playSound(l__GameSound__5.ARMOR_UNEQUIP);
	...
	function v9.OnSecondaryBtnClick()
		l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):closeLayer(l__UILayers__14.OVERLAY);
	end;
	l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):openApp(l__BedwarsAppIds__3.CONFIRMATION_MODAL, v9);
end;
...
function v5.clanInviteDecision(p33, p34, p35)
	p33.clansRemotes:Get("ClanInviteDecision"):CallServerAsync(p34, p35):andThen(function(p36)
		...
		if v10 ~= "" and v10 and not l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):isAppOpen(l__BedwarsAppIds__3.CLAN_PROFILE) then
			l__Flamework__2.resolveDependency("@easy-games/game-core:client/controllers/app-controller@AppController"):openApp(l__BedwarsAppIds__3.CLAN_PROFILE, {
				ClanId = p36.clanId
			});
		end;
