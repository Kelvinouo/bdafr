...
local l__LumenProgressBar__9 = v1.import(script, script.Parent, "ui", "lumen-progress-bar").LumenProgressBar;
function u1.KnitStart(p2)
	...
	l__KnitClient__5.Controllers.KitController:watchLocalKit(function(p5)
		p2.maid:DoCleaning();
		if p5 ~= l__BedwarsKit__6.LUMEN then
			return nil;
		end;
		p2.maid:GiveTask((l__Flamework__7.resolveDependency("@easy-games/game-core:client/controllers/action-bar/action-bar-controller@ActionBarController"):addComponent(l__BuildRoduxApp__8(l__LumenProgressBar__9))));
	end);
end;
...
local l__GameQueryUtil__12 = v1.import(script, v1.getModule(script, "@easy-games", "game-core").out).GameQueryUtil;
...
function u1.createEffect(p6, p7)
	...
	local v8 = l__ReplicatedStorage__11.Assets.Effects.LumenUpgrade:Clone();
	local v9 = { v8 };
	...
	while true do
		...
		v13 = v11;
		l__GameQueryUtil__12:setQueryIgnored(v12, true);	
	end;
...
function u1.playAscendEffect(p8, p9)
	local v14 = l__ReplicatedStorage__11.Assets.Effects.LumenAscend:Clone();
