...
function u1.KnitStart(p2)
	...
	l__ClientStore__4.changed:connect(function(p3, p4)
		...
		if l__selectedTurret__8 == p4.Game.selectedTurret then
			return nil;
		end;
		p2.maid:DoCleaning();
		if l__selectedTurret__8 == nil or table.find(l__CollectionService__5:GetTags(l__selectedTurret__8), l__TurretId__6.VULCAN_TURRET) == nil then
			return nil;
		end;
		p2.maid:GiveTask(l__UserInputService__7.InputBegan:Connect(function(p5, p6)
			if not p6 and p2:isAcceptableInput(p5) and p5.UserInputState == Enum.UserInputState.Begin then
				p2.mouseDown = true;
			end;
		end));
		p2.maid:GiveTask(l__UserInputService__7.InputEnded:Connect(function(p7, p8)
			if not p8 and p2:isAcceptableInput(p7) and p7.UserInputState == Enum.UserInputState.End then
				p2.mouseDown = false;
			end;
		end));
		p2.maid:GiveTask(l__RunService__8.Heartbeat:Connect(function()
			if p2.mouseDown == true then
				p2:fire(l__selectedTurret__8);
			end;
		end));
		p2.maid:GiveTask(l__UserInputService__7.TouchTapInWorld:Connect(function(p9, p10)
			if not p10 then
				p2:fire(l__selectedTurret__8);
			end;
		end));
		l__CollectionService__5:GetInstanceAddedSignal(l__TurretId__6.VULCAN_TURRET):Connect(function(p11)
			p11:SetAttribute("ProjectilesIgnore", true);
			for v9, v10 in ipairs(p11:GetDescendants()) do
				l__GameQueryUtil__9:setQueryIgnored(v10, true);
			end;
		end);
	end);
	l__default__10.Client:OnEvent("RemoteName", function(p12)
		if p12.extra.projectileData and p12.extra.projectileData.projectileType == "turretBullet" and p12.entityInstance == l__Players__11.LocalPlayer.Character then
			local v11 = u3.new();
...
local l__HapticService__16 = v4.HapticService;
