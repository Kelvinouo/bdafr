...
function v6.KnitStart(p2)
	...
	l__ClientSyncEvents__7.SwordChargedSwing:connect(function(p3)
		if p3.weapon.Name ~= l__ItemType__4.HELL_SABER then
			return nil;
		end;
		if p3.chargedAttack.chargeTime >= 0.5 then
			...
			local v8 = false;
			if p3.chargedAttack.chargeTime >= 1.5 then
				v8 = p3.fromPlayer:GetAttribute("EmberUpgraded");
			end;
			if v8 ~= 0 and v8 == v8 and v8 ~= "" and v8 then
				...
				v1.Promise.defer(function()
					...
					while true do
						...
						local v11 = l__EntityUtil__9:getEntity(p3.fromPlayer);
						if v11 ~= nil then
	...
	l__ClientSyncEvents__7.SwordCharge:connect(function(p4)
		if p4.itemType.itemType ~= l__ItemType__4.HELL_SABER then
			return nil;
		end;
		...
		if p4.chargeState == l__ChargeState__10.Charging then
			...
			local v14 = l__getItemMeta__12(p4.itemType.itemType).sword;
			if v14 ~= nil then
...
function v6.isRelevantItem(p5, p6)
	return p6.itemType == l__ItemType__4.HELL_SABER;
end;
...
function v6.playSound(p10)
	p10.chargingSound = l__SoundManager__16:playSound(l__GameSound__6.INFERNAL_SWORD_CHARGE);
	if p10.chargingSound then
		p10.animationMaid:GiveTask(function()
			local l__chargingSound__16 = p10.chargingSound;
...
function v6.playEffects(p11, p12)
	local v17 = l__EntityUtil__9:getEntity(p12);
	if v17 == nil then
		return nil;
	end;
	...
	local u25 = 0;
	local u26 = l__RunService__23.Heartbeat:Connect(function(p13)
		local v22 = v17:getInstance():GetPrimaryPartCFrame();
		if not p11:validPlayerConditions(v17) then
			v18:DoCleaning();
			return nil;
		end;
		u25 = u25 + p13;
...
function v6.validPlayerConditions(p14, p15)
	if not p15:isAlive() then
		return false;
	end;
	local v23 = p15:getItemInHand();
	if v23 ~= nil then
		v23 = v23.Name;
	end;
	...
	if p15:getInstance().PrimaryPart == nil then
		return false;
	end;
	return true;
end;
