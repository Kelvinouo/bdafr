...
function u1.KnitStart(p2)
	...
	l__ClientSyncEvents__3.ProjectileLaunched:connect(function(p3)
		if p3:isLocalShooter() and p3.projectileType == "fish_net" then
			l__Flamework__4.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):setOnCooldown(l__CooldownId__5.FISH_NET_THROW, l__getItemMeta__6(l__ItemType__7.FISH_NET).projectileSource.fireDelaySec);
		end;
	end);
end;
function u1.isRelevantItem(p4, p5)
	return p5.itemType == l__ItemType__7.FISH_NET;
end;
function u1.onEnable(p6, p7, p8)
	v2.onEnable(p6, p7, p8);
	p6:setupYield(function()
		local v5 = u2.new();
		v5:GiveTask(l__Flamework__4.resolveDependency("@easy-games/game-core:client/controllers/cooldown/cooldown-controller@CooldownController"):createCooldownBar(l__CooldownId__5.FISH_NET_THROW));
		return function()
			v5:DoCleaning();
		end;
	end);
