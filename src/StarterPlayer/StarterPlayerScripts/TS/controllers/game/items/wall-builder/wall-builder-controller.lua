-- Decompiled with the Synapse X Luau decompiler.

local v1 = require(game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include"):WaitForChild("RuntimeLib"));
local l__KnitController__2 = v1.import(script, script.Parent.Parent.Parent.Parent.Parent, "lib", "knit", "knit-controller").KnitController;
local v3 = setmetatable({}, {
	__tostring = function()
		return "WallBuilderController";
	end, 
	__index = l__KnitController__2
});
v3.__index = v3;
function v3.new(...)
	local v4 = setmetatable({}, v3);
	return v4:constructor(...) and v4;
end;
function v3.constructor(p1)
	l__KnitController__2.constructor(p1);
	p1.Name = "WallBuilderController";
end;
local l__BlockEngineClientEvents__1 = v1.import(script, v1.getModule(script, "@easy-games", "block-engine").out.client["block-engine-client-events"]).BlockEngineClientEvents;
local l__Workspace__2 = v1.import(script, v1.getModule(script, "@rbxts", "services")).Workspace;
local l__default__3 = v1.import(script, game:GetService("ReplicatedStorage"), "TS", "remotes").default;
function v3.KnitStart(p2)
	l__KnitController__2.KnitStart(p2);
	l__BlockEngineClientEvents__1.PlaceBlock:connect(function(p3)
		local v5 = #p3.blockType - 7;
		local v6 = v5 <= 0 or string.sub(p3.blockType, v5) ~= "_builder";
		if v6 then
			return nil;
		end;
		local l__LookVector__7 = l__Workspace__2.CurrentCamera.CFrame.LookVector;
		if math.abs(l__LookVector__7.X) - math.abs(l__LookVector__7.Z) > 0 then
			local v8 = Vector3.new(0, 0, 1);
		else
			v8 = Vector3.new(1, 0, 0);
		end;
		l__default__3.Client:Get("RemoteName"):SendToServer({
			blockPosition = p3.blockPosition, 
			direction = v8, 
			blockType = p3.blockType
		});
	end);
end;
local v9 = v1.import(script, v1.getModule(script, "@easy-games", "knit").src).KnitClient.CreateController(v3.new());
return nil;
