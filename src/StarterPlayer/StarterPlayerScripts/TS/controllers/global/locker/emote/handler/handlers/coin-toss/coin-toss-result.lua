...
return {
	CoinTossResult = v1.import(script, v1.getModule(script, "@rbxts", "roact-hooks").src).new(u1)(function(p1, p2)
		...
		p2.useEffect(function()
			...
			task.delay(1.5, function()
				local l__guiTree__6 = l__KnitClient__4.Controllers.CoinTossHandlerController.guiTree;
				if l__guiTree__6 then
					u7:Play();
					u8:Play();
					u9:GiveTask(u8.Completed:Connect(function()
						u1.unmount(l__guiTree__6);
					end));
				end;
			end);
			return function()
		...
		return u1.createFragment({
			CoinTossEmoteGui = u1.createElement("BillboardGui", {
				Size = UDim2.fromScale(0, 0), 
				StudsOffsetWorldSpace = Vector3.new(0, 2.5, 0), 
				Adornee = p1.Character.Head, 
				[u1.Ref] = u6
			}, {
				CoinTossEmoteResult = u1.createElement("Frame", {
