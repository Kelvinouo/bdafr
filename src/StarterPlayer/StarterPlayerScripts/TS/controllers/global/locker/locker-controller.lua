...
function u1.KnitStart(p2)
	p2.namespace:WaitFor("RemoteName"):andThen(function(p3)
		l__ClientStore__4:dispatch({
			type = "LockerUpdateAll", 
			locker = p3:CallServer()
		});
	end);
	p2.namespace:WaitFor("RemoteName"):andThen(function(p4)
...
function u1.setSpray(p8, p9)
	if p8.namespace:Get("RemoteName"):CallServer({
		spray = p9
	}) then
		l__ClientStore__4:dispatch({
			type = "LockerSetSpray", 
...
function u1.setKillEffect(p10, p11)
	if p10.namespace:Get("RemoteName"):CallServer({
		killEffect = p11
	}) then
		l__ClientStore__4:dispatch({
...
function u1.setTitle(p12, p13)
	if p12.namespace:Get("RemoteName"):CallServer({
		title = p13
	}) then
		l__ClientStore__4:dispatch({
			type = "LockerSetTitle", 
...
function u1.setLobbyGadget(p14, p15)
	if p14.namespace:Get("RemoteName"):CallServer({
		lobbyGadget = p15
	}) then
		l__ClientStore__4:dispatch({
